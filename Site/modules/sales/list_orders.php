...
// FINAL version with improved Date Range filter and layout.
...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <form id="orderSearchForm" class="row gx-3 gy-2 align-items-center">
                ...
                <!-- REPLACED: Two date inputs with one date range picker input -->
    ...
    <!-- Order List Table -->
    <div class="card">
        <div class="card-body"><div class="table-responsive"><table class="table table-hover"><thead class="table-light"><tr><th>Order ID</th><th>Date</th><th>Customer</th><th>Phone</th><th class="text-end">Total Amount</th><th>Order Status</th><th>Payment Status</th><th>Actions</th></tr></thead><tbody id="orderListTableBody"><?php if (empty($initial_orders)): ?><tr><td colspan="8" class="text-center text-muted">No orders found.</td></tr><?php else: ?><?php foreach ($initial_orders as $order): ?><tr><td><?= htmlspecialchars($order['order_id']) ?></td><td><?= htmlspecialchars(date("d-m-Y", strtotime($order['order_date']))) ?></td><td><?= htmlspecialchars($order['customer_name']) ?></td><td><?= htmlspecialchars($order['customer_phone']) ?></td><td class="text-end"><?= htmlspecialchars(number_format($order['total_amount'], 2)) ?></td><td><span class="badge bg-info text-dark"><?= htmlspecialchars($order['status']) ?></span></td><td><span class="badge bg-<?= $order['payment_status'] == 'Received' ? 'success' : 'warning' ?>"><?= htmlspecialchars($order['payment_status']) ?></span></td><td><a href="/modules/sales/entry_order.php?order_id=<?= htmlspecialchars($order['order_id']) ?>" class="btn btn-sm btn-outline-primary"><i class="bi bi-pencil"></i> View</a></td></tr><?php endforeach; ?><?php endif; ?></tbody></table></div></div>
    </div>
</main>
