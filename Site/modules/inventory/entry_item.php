...
// FINAL version with a single form and corrected image management logic.
// 20251012 1625
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Determine the action from the hidden input, default to saving details.
    $action = $_POST['action'] ?? 'save_details';
    $posted_id = $_POST['item_id'] ?? '';

    try {
        if (!$posted_id && in_array($action, ['delete_image', 'set_main_image'])) {
            throw new Exception("An Item ID is required to manage images.");
        }

        switch ($action) {
            case 'delete_image':
                $image_path_to_delete = $_POST['image_path'] ?? '';
                if (empty($image_path_to_delete)) throw new Exception("Image path is missing.");
                ...
                if (!file_exists($full_server_path)) throw new Exception("Image file not found.");
                
                $stmt_check_main = $db->prepare("SELECT main_image_path FROM items WHERE item_id = ?");
                $stmt_check_main->bind_param("s", $posted_id);
                ...
                if ($current_item && $current_item['main_image_path'] === $image_path_to_delete) {
                    ...
                    $stmt_clear_main->bind_param("s", $posted_id);
                    $stmt_clear_main->execute();
                }

                if (!unlink($full_server_path)) throw new Exception("Failed to delete the image file.");
            ...
            case 'set_main_image':
                ...
                $stmt->bind_param("ss", $image_path_to_set, $posted_id);
                $stmt->execute();
            ...
            case 'save_details':
                ...
                if (!$stmt->execute()) throw new Exception("Database error: Failed to save item details.");
                ...
                if (isset($_FILES['new_images']) && !empty($_FILES['new_images']['name'][0])) {
                    ...
                    if ($existing_images) {
                        foreach ($existing_images as $img) {
                            ...
                            if ($matches && (int)$matches[1] > $last_num) $last_num = (int)$matches[1];
                        }
                    }
                    ...
                    foreach ($_FILES['new_images']['tmp_name'] as $key => $tmp_name) {
                        if ($_FILES['new_images']['error'][$key] === UPLOAD_ERR_OK) {
                            ...
                            if (!move_uploaded_file($tmp_name, $image_dir . $new_filename)) throw new Exception("Failed to move uploaded image.");
                ...
                $_SESSION['success_message'] = "✅ Item successfully $form_action.";
                $posted_id = $action_item_id; // Ensure redirect uses the new ID if created
                break;
        }

        header("Location: entry_item.php?item_id=" . $posted_id);
        exit;
...
<main class="container mt-4">
    ...
    <!-- There is now ONLY ONE form on the page -->
    <form method="POST" class="needs-validation mb-5" novalidate id="itemForm" enctype="multipart/form-data" action="entry_item.php<?= $is_edit ? '?item_id=' . htmlspecialchars($item['item_id']) : '' ?>">
        <input type="hidden" name="item_id" value="<?= htmlspecialchars($item['item_id'] ?? '') ?>">
        <!-- NEW: Hidden inputs for JavaScript to control image actions -->
        <input type="hidden" name="action" id="imageAction" value="save_details">
        ...
        <div class="row g-3">
            ...
            <div class="card col-12 mt-4">
                ...
                <div class="card-body" id="imageManagementSection">
                    <?php if ($is_edit): ?>
                    <div class="mb-3">
                                ...
                                <?php foreach ($item['images'] as $image_url): ?>
                                    <div class="col-auto text-center">
                                        ...
                                        <div class="d-flex justify-content-center gap-1 mt-1">
                                            <!-- CORRECTED: Buttons are now type="button" and use data attributes -->
                                            ...
                                            <?php else: ?>
                                                <button type="button" class="btn btn-sm btn-outline-success btn-set-main" title="Set as Main Image" data-image-path="<?= htmlspecialchars($image_url) ?>">
                                                    <i class="bi bi-check-circle"></i>
                                                </button>
                                            <?php endif; ?>
                                            <button type="button" class="btn btn-sm btn-outline-danger btn-delete-image" title="Delete Image" data-image-path="<?= htmlspecialchars($image_url) ?>">
