...
// --- AJAX Endpoint 1: Get Sub-Categories for a Category ---
...
// --- AJAX Endpoint 2: Live Search for Items ---
...
// Fetch all parent categories for the first dropdown
$parent_categories = get_all_categories();
$sub_categories = []; // Will be populated in edit mode

// Initialize default item structure
$item = [
    'item_id' => '',
    'category_id' => '', // For pre-selecting the parent category dropdown
    'category_sub_id' => '',
    'name' => '',
    'description' => '',
...
if (isset($_GET['item_id'])) {
    ...
    if ($item_data) {
        ...
        $is_edit = true;
        // Pre-load the sub-categories for the selected parent category
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($is_edit_post) {
            ...
            $stmt = $db->prepare("UPDATE items SET name = ?, category_sub_id = ?, description = ?, updated_at = NOW(), updated_by = ?, updated_by_name = ? WHERE item_id = ?");
            $stmt->bind_param("sssiss", $name, $category_sub_id, $description, $current_user_id, $current_user_name, $posted_id);
            $action = 'updated';

        } else {
            // ----- CREATE new item -----
            $item_id = generate_sequence_id('item_id', 'items', 'item_id');
            $stmt = $db->prepare("INSERT INTO items (item_id, category_sub_id, name, description, created_at, created_by, created_by_name) VALUES (?, ?, ?, ?, NOW(), ?, ?)");
            $stmt->bind_param("ssssis", $item_id, $category_sub_id, $name, $description, $current_user_id, $current_user_name);
            $action = 'created';
        }
...
<main class="container mt-4">
    ...
    <form method="POST" class="row g-3 needs-validation" novalidate id="itemForm" action="entry_item.php<?= $is_edit ? '?item_id=' . htmlspecialchars($item['item_id']) : '' ?>">
            ...
            <div class="input-group">
                <select class="form-select" id="category_id" name="category_id" required>
                    ...
                    <?php foreach ($parent_categories as $parent): ?>
                        <option value="<?= htmlspecialchars($parent['category_id']) ?>" <?= ($item['category_id'] == $parent['category_id']) ? 'selected' : '' ?>>
                            <?= htmlspecialchars($parent['name']) ?>
                        </option>
                    <?php endforeach; ?>
                </select>
                <a href="entry_category.php" class="btn btn-success" title="Add New Category" target="_blank"><i class="bi bi-plus-lg"></i></a>
            </div>
        </div>

        <div class="col-md-6">
            <label for="category_sub_id" class="form-label">Sub-Category *</label>
            <div class="input-group">
                <select class="form-select" id="category_sub_id" name="category_sub_id" required <?= $is_edit ? '' : 'disabled' ?>>
                    <option value="">Choose Sub-Category...</option>
                    <?php foreach ($sub_categories as $sub): ?>
                        <option value="<?= htmlspecialchars($sub['category_sub_id']) ?>" <?= ($item['category_sub_id'] == $sub['category_sub_id']) ? 'selected' : '' ?>>
                            <?= htmlspecialchars($sub['name']) ?>
                        </option>
                    <?php endforeach; ?>
                </select>
                <a href="entry_category_sub.php" class="btn btn-success" title="Add New Sub-Category" target="_blank"><i class="bi bi-plus-lg"></i></a>
        ...
        <div class="col-12">
            <button class="btn btn-primary" type="submit"><i class="bi bi-<?= $is_edit ? 'floppy' : 'plus-circle' ?>"></i> <?= $is_edit ? 'Update' : 'Create' ?> Item</button>
